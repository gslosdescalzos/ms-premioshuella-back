...
def create_vote(
    db: Session, user_id: int, category_id: int, participant_id: int
) -> InitialVote:
    participation = (
        db.query(Participation)
        .filter(
            Participation.user_id == participant_id,
    ...
    if participation is None:
        raise HTTPException(
            ...
            detail="Participation not found for this participant in this category",
        )

    existing_vote = (
        db.query(InitialVote)
        .filter(
            InitialVote.user_id == user_id,
    ...
    if existing_vote is not None:
        raise HTTPException(
            ...
            detail="User has already voted in this category",
        )
...
def get_votes_by_category(db: Session) -> list[dict]:
    results = (
        db.query(
            ...
            User.username.label("participant_username"),
